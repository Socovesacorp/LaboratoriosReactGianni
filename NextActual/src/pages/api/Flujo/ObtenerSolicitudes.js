...
export default async function handler(req, res) {
  if (req.method === 'GET') {
    ...
    try {
      ...
      const { FOLIO, NOMBREAUTOMATICO_SOLICITANTE, TAREA, NOMBRE_BENEFICIARIO } = req.query;
      ...
      if (FOLIO) {
        query += `FOLIO = '${FOLIO}' AND `;
      }
      if (NOMBREAUTOMATICO_SOLICITANTE) {
        query += `NOMBREAUTOMATICO_SOLICITANTE LIKE '%${NOMBREAUTOMATICO_SOLICITANTE}%' AND `;
      }
      if (TAREA) {
        query += `TAREA LIKE '%${TAREA}%' AND `;
      }
      if (NOMBRE_BENEFICIARIO) {
        query += `NOMBRE_BENEFICIARIO LIKE '%${NOMBRE_BENEFICIARIO}%' AND `;
      }

      // Eliminar el "AND" extra al final de la consulta
