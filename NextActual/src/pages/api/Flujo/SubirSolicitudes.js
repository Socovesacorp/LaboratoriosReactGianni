  ...
  if (req.method === 'POST') {
    try {
      ...
      try {
        ...
        for (const record of recordsToInsert) {
          ...
          if (existeFolio) {
            ...
            await transaction.request()
              ...
              .input('CENTRO_DE_COSTO'                        ,sql.VarChar(500),         CENTRO_DE_COSTO                      )
        ...
        console.error('11Error al insertar registros:'+folio, error);
        res.status(500).json({ error: '22Error al insertar registros*'+folio, details: error.message });
      }
    } catch (error) {
      console.error('33Error al insertar registros:'+folio, error);
      res.status(500).json({ error: '44Error al insertar registros'+folio, details: error.message });
    }
  } else {
